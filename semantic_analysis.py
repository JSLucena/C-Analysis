...
def query_for_loop_out_of_bounds(tree, code):
    ...
    if 'size_var' in loop_info and 'offset' in loop_info:
        return(f"Warning: Potential out-of-bounds access detected! Loop accesses beyond allocated buffer size by offset {loop_info['offset']}.")
    else:
        return("ok")
...
def check_for_out_of_bounds1(for_parts, array_parts):
    ...
    if 'loop_limit' in for_parts and 'size' in array_parts:
        ...
        if loop_limit >= array_size:
            return("Warning: Potential out-of-bounds access detected! Loop limit exceeds buffer size.")
        else:
            return("ok")
    else:
        return("ok")
...
def detect_array_access(tree, code, array_info):
    ...
    for node, capture_name in captures:
        if capture_name == 'array_name' and code[node.start_byte:node.end_byte].strip() == array_info.get('name'):
            ...
            if access_index < 0:
                return(f"  Warning: Buffer under-read detected! Access index {access_index} is below array start.")
            elif access_index >= array_size:
                return(f"  Warning: Out-of-bounds read detected! Access index {access_index} exceeds array size {array_size}.")
            else:
                return("ok")
    #print()
    return "ok"
        ...
        for loop_info in loops_info:
            ...
            if loop_var and loop_limit:
                ...
                if loop_limit_value > dimensions[0]:
                    return(f"  Warning: Out-of-bounds access detected in '{array_name}' array! Loop limit {loop_limit_value} exceeds array size {dimensions[0]}.")
    return "ok"
...
def detect_multi_out_of_bounds_access(tree, code):
    ...
    for array in array_info:
        ...
        for call_index, index in enumerate(calls):
            ...
            if index < 0:
                return(f"  Warning: Buffer under-read detected! Index {index} is below 0 for array '{array_name}'.")
            elif index >= array_size:
                return(f"  Warning: Out-of-bounds read detected! Index {index} exceeds array size {array_size} for array '{array_name}'.")
            else:
                return(f"ok")
...
def detect_buffer_overflow_and_overread(tree, code):
    ...
    if buffer_size is not None and length_value is not None and length_value > buffer_size:
        return(f"Warning: Potential buffer overflow! The length ({length_value}) exceeds localBuffer size ({buffer_size}).")
    
    if length_value is not None and length_value < 0:
        return("Warning Negative length causes under-read")
    ...
    if data_size is not None and length_value > data_size:
        return(f"Warning: Potential buffer over-read! The length ({length_value}) exceeds data size ({data_size}).")
...
def detect_dynamic_overflow_in_loop(tree, code):
    ...
    if not malloc_size and not realloc_info and not final_size_arg:
        ...
        return  "ok"# If no data is available, skip detection
    ...
    if final_size_arg and malloc_size[0] is not None:
        try:
            ...
            return "ok" # Unable to convert, skip detection
    else:
        if realloc_info:
            try:
    ...
    if loop_limit is not None:
        if loop_limit > int(malloc_size[0]):
            return("Warning: Potential buffer overflow detected! Loop limit exceeds allocated buffer size.")
        else:
            return("ok")
...
def detect_memcpy_overflow_with_src(tree, code):
    ...
    if total_copy_length > dest_size:
        return(f"Warning: Potential buffer overflow in memcpy! Trying to copy {total_copy_length} bytes into {dest_size}-byte destination buffer.")
    else:
        return("ok")
...
def check_memcpy_overflow(memcpy_info, arrays_info):
    ...
    if dest_size < src_size:
        return(f"  Warning: Potential overflow in memcpy detected! Trying to copy {src_size} bytes into {dest_size}-byte destination buffer.")
    else:
        return("ok")
...
def detect_realloc_overflow(tree, code):
    ...
    if malloc_size is not None and realloc_size is not None and loop_limit is not None:
        if loop_limit > malloc_size:
            return("Warning: Potential overflow detected! Loop limit exceeds initial malloc size.")
        
        if loop_limit > realloc_size:
            return("Warning: Potential overflow detected! Loop limit exceeds reallocated buffer size.")
...
def detect_heap_overflow(malloc_info, write_info):
    ...
    if alloc_size < src_length:
        return(f"  Warning: Potential heap buffer overflow detected! Attempt to write {src_length} bytes into {alloc_size}-byte buffer.")
    else:
        return("ok")
...
def detect_double_free(malloc_vars, free_vars):
    ...
    for var in free_vars:
        if var not in allocated_vars:
            return(f"  Warning: Freeing unallocated variable '{var}'.")
        elif var in freed_vars:
            return(f"  Warning: Double free detected for variable '{var}'.")
        else:
            freed_vars.add(var)
...
def detect_use_after_free(malloc_vars, free_vars, dereference_vars):
    ...
    for var in dereference_vars:
        if var in freed:
            return(f"  Warning: Potential use after free detected for variable '{var}'.")
    ...
    for node, capture_name in captures:
        if capture_name == 'var_name':
            ...
            if var_name in freed_vars:
                return(f"Warning: Use-after-free detected for variable '{var_name}' in a conditional block.")
            else:
...
def detect_return_of_stack_variable(stack_vars, return_var):
    ...
    if return_var and return_var in stack_vars:
        return(f"Warning: Returning local stack variable '{return_var}', which may lead to invalid memory access.")
    else:
        return("ok")
...
def detect_invalid_return(arrays_info, return_vars):
    ...
    for array in arrays_info:
        if array["name"] in return_vars:
            return(f"  Warning: Returning local stack-allocated array '{array['name']}' from function. This can lead to undefined behavior.")
    return "ok"
    ...
    for count_var, data_type in malloc_calls:
        ...
        if count_value is not None and sizeof_value is not None:
            ...
            if total_size > MAX_32BIT_SIZE:  
                #return(f"Warning: Potential integer overflow detected for count variable '{count_var}', allocation size {total_size} exceeds 32-bit limit.")
...
def detect_integer_overflow_and_out_of_bounds(tree, code):
    ...
    if malloc_info:
        ...
        if count_value * size_value > int_max:
            return(f"Warning: Potential integer overflow detected! count * size exceeds 32-bit integer limit.")
        ...
        if allocation_size > int_max or allocation_size <= 0:
            return("Warning: Allocation failed due to overflow, leading to potential out-of-bounds write.")
        else:
            return("ok")
    else:
        return("ok")
...
def main_detection_flow(tree, code):
    ...
    if overflow_detected:
        return("Potential integer overflow detected that could lead to buffer overflow.")
    ...
    ret = detect_heap_overflow(malloc_info, write_info)
    if ret != "ok":
        return ret
    ...
    if array_parts and for_parts:
        ret = check_for_out_of_bounds1(for_parts, array_parts[0])
        if ret != "ok":
            return ret
    ...
    if array_parts:
        ret = detect_array_access(tree, code, array_parts[0])
        if ret != "ok":
            return ret
    ...
    ret = detect_double_free(malloc_vars, free_vars)
    if ret != "ok":
        return ret
    
    # Detect use after free
    dereference_vars = query_dereference(tree, code)
    ret = detect_use_after_free(malloc_vars, free_vars, dereference_vars)
    if ret != "ok":
        return ret

    # Detect invalid return of local arrays
    return_vars = query_return_statements(tree, code)
    ret = detect_invalid_return(array_parts, return_vars)
    if ret != "ok":
        return ret
    ...
    if memcpy_info:
        ret = check_memcpy_overflow(memcpy_info, arrays_info)
        if ret != "ok":
            return ret
    ...
    ret = detect_overflow_in_multidimensional_arrays(multi_arrays_info, multi_loops_info, len_value)
    if ret != "ok":
        return ret

    ret = detect_realloc_overflow(tree, code)
    if ret != "ok":
        return ret
    ret = detect_multi_out_of_bounds_access(tree, code)
    if ret != "ok":
        return ret
    # Call detection functions
    ret = detect_buffer_overflow_and_overread(tree, code)
    if ret != "ok":
        return ret

    ret = detect_use_after_free_in_conditional(tree, code)
    if ret != "ok":
        return ret

    stack_vars, return_var = query_stack_variables_and_return(tree, code)

    ret = detect_return_of_stack_variable(stack_vars, return_var)
    if ret != "ok":
        return ret

    ret =detect_integer_overflow_and_out_of_bounds(tree, code)
    if ret != "ok":
        return ret

    ret = detect_dynamic_overflow_in_loop(tree, code)
    if ret != "ok":
        return ret

    ret = query_for_loop_out_of_bounds(tree, code)
    if ret != "ok":
        return ret

    ret =detect_memcpy_overflow_with_src(tree, code)
    if ret != "ok":
        return ret
    ...
    for c_file, functions in tqdm(c_file_functions.items(), desc=f"Processing Files in {directory}", leave=False):
        ...
        with open(c_file, 'r') as file:
            ...
            for function in functions:
                code = extract_function(f, function)
                #print(code)
                
                try:
                    tree = parser.parse(bytes(code, "utf8"))
                    ret = main_detection_flow(tree, code)
...
with open("semantic_results.json", "w") as json_file:
